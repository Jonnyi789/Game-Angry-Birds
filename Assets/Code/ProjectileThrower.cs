...
public class ProjectileThrower : MonoBehaviour {
    ...
    /// <summary>
    /// Cached copy of the RigidBody2D so we don't have to keep looking it up. Faster
    /// </summary>
    ...
    {
        foreach (Rigidbody2D thing in (FindObjectsOfType<Rigidbody2D>()))
        {
            if ((thing.gameObject.GetComponent<Renderer>().isVisible) && (IsActive(thing))) //Or I can use IsOffScreen here.
                return true;
        }
        return false;
    }
    ...
    /// <summary>
    /// Reload the current level (Next step: make more levels)
    /// </summary>
